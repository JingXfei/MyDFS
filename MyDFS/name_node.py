...
class NameNode:
    def run(self):  # 启动NameNode
        # 创建一个监听的socket
        listen_fd = socket.socket()                         #创建socket实例
        try:
            # 监听端口
            listen_fd.bind(("0.0.0.0", name_node_port))     #bind()函数把一个地址族中的特定地址赋给socket
            listen_fd.listen(5)                             #开始监听，最多同时连接五个客户端
            print("Name node started")
            while True:
                # 等待连接，连接后返回通信用的套接字
                sock_fd, addr = listen_fd.accept()#TCP客户端依次调用socket()、connect()之后就向TCP服务器发送了一个连接请求。
                ...
                try:
                    ...
                    if cmd == "ls":  # 若指令类型为ls, 则返回DFS上对于文件、文件夹的内容
                        ...
                        local_path=request[1]
                        dfs_path = request[2]  # 指令第二个参数为DFS目标地址
                        file_size = int(request[3])
                        response = self.new_fat_item(local_path,dfs_path, file_size)
    ...
    def new_fat_item(self, local_path,dfs_path, file_size):
        nb_blks =int(math.ceil(file_size / dfs_blk_size))
        #***********************防止隔断传输*********************************
        fp = open(local_path)
        size_of_file=file_size
        size_each_step=[]
        print('进入到new_fat_item')
        for i in range(int(size_of_file / dfs_blk_size)):  # 216/20:10
            data =fp.read(int(dfs_blk_size))
            step_size=dfs_blk_size
            if data[-1] != '\n':
                node_read = '1'
                while node_read != '\n' and size_of_file >= 0:
                    node_read = fp.read(1)
                    data = data + node_read
                    step_size+=1
            size_of_file -= step_size
            size_each_step.append(step_size)
        gap = size_of_file
        size_each_step.append(gap)
        print('gap',size_each_step)
        ...
        for i in range(nb_blks):
            ...
            #*************************************************************
            # if blk_size==dfs_blk_size:
            
            data_pd.loc[i] = [blk_no, host_name, size_each_step[i]]
            
        print('data_pd.loc',data_pd)
        # 获取本地路径
        local_path = name_node_dir + dfs_path
